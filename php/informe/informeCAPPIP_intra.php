<?php
    ...
    $pdf->RowPIP($nombres_casos_clinicos);
    ...
    $pdf->SetHeightRow(4);
    $pdf->SetFillColorsRow($coloresFondo);
    $pdf->RowPIP($nombres_respuestas_correctas);
    ...
    for($count_pat=0; $count_pat<sizeof($usuarios_pat) ; $count_pat++){
        ...
        for($o=0; $o<sizeof($casos_clinicos); $o++){
            ...
            if(is_numeric($search_cod = array_search($usuarios_pat[$count_pat]["cod_usuario"], array_column($respuestas_patologos, "cod_patologo")))){ // Si encuentra el patologo en la lista de intentos
                ...
                $criterioAct = $pdf->obtenerCriterioPIP($id_respuesta_pat, $id_respuestas_correctas);
    ...
    if(!($diagnosticos_concordantes == 0 && $diagnosticos_noconcordantes == 0)){
        ...
        $ruta_image = $pdf->generarGraficoPIP($diagnosticos_concordantes,$diagnosticos_noconcordantes);
    ...
    while($qryData = mysql_fetch_array($qryArray)) {
        ...
        $pdf->setX($left_convenciones);
        $pdf->RowPIP(["Historia clínica", $historia_clinica]);
        ...
        $pdf->setX($left_convenciones);
        $pdf->RowPIP(["Todos los distractores del caso clínico"]);
        ...
        $pdf->SetGrossLineRightRow([false, true]);
        $pdf->setX($left_convenciones);
        $pdf->RowPIP(["Abreviatura", "Descripción"]);
    ...
    // Cerrar PDF
    $pdf->Close();
    $nomArchivo = utf8_decode("$num_laboratorio - Intralaboratorio CAP-PIP $pdf->nom_reto $nom_laboratorio.pdf");
