<?php
    ...
    for($count_pat=0; $count_pat<sizeof($usuarios_pat) ; $count_pat++){
        ...
        for($o=0; $o<sizeof($casos_clinicos); $o++){
            ...
            if(is_numeric($search_cod = array_search($usuarios_pat[$count_pat]["cod_usuario"], array_column($respuestas_patologos, "cod_patologo")))){ // Si encuentra el patologo en la lista de intentos
                ...
                $criterioAct = $pdf->obtenerCriterioCITLBC($valor_respuesta_pat);
                ...
                if($criterioAct == 2){ // concordante
                    ...
                    $colorFondoAct = $pdf->amarilloOscuro;
        ...
        */
        ...
        for($i=0;$i<count($array_NR);$i++) {
            ...
            } else if ($array_FCR[$i] == $pdf->amarilloOscuro){
                   
                $pdf->SetFont("Arial","B",7);
                $pdf->SetTextColor(255, 201, 17); //Amarillo
                $pdf->MultiCell($w,$pdf->heightRow,$array_NR[$i],0,$a,1);
    ...
    //     $ruta_image = $pdf->generarGraficoCITLBC(
    //         $diagnosticos_concordantes,
    //         $diagnosticos_discordanciamenor,
    //         $diagnosticos_noconcordantes
    //     );
    ...
    // $pdf->Cell(35,4,"COD.\nPatólogo",1,0,'C',1);
    // $pdf->Cell(24,4,"#Concordantes",1,0,'C',1);
    // $pdf->Cell(24,4,"#Discordancia menor",1,0,'C',1);
    // $pdf->Cell(24,4,"#No concordantes",1,0,'C',1);
    ...
    //     $pdf->Cell(35,4,$usuarios_pat[$count_pat]["cod_usuario"],1,0,'C',1);

    //     $pdf->SetFont("Arial","",6);
    //     $pdf->Cell(24,4,$usuarios_pat[$count_pat]["concordantes"],1,0,'C',1);
    //     $pdf->Cell(24,4,$usuarios_pat[$count_pat]["discordancia_menor"],1,0,'C',1);
    //     $pdf->Cell(24,4,$usuarios_pat[$count_pat]["no_concordantes"],1,0,'C',1);
    // }
    ...
    // $pdf->SetFont("Arial","",6);
    // $pdf->Cell(24,4,$pdf->contarNumsArray(array_column($usuarios_pat,"concordantes")),1,0,'C',1);
    // $pdf->Cell(24,4,$pdf->contarNumsArray(array_column($usuarios_pat,"discordancia_menor")),1,0,'C',1);
    // $pdf->Cell(24,4,$pdf->contarNumsArray(array_column($usuarios_pat,"no_concordantes")),1,0,'C',1);

    if($pdf->GetY() > $max_height_break){ // Si el alto de la tabla es mayor al de los 
        $max_height_break = ($pdf->GetY() + 20);
    }
    ...
    // Cerrar PDF
    $pdf->Close();
    $nomArchivo = utf8_decode("$num_laboratorio - Intralaboratorio CITLBC $pdf->nom_reto $nom_laboratorio.pdf");
